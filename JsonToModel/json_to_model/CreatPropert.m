...
+ (void)myNSMutableDictionary:(NSString *)tempStr ToNSMutableString:(NSMutableString *)StrM{
    [StrM appendFormat:@"@property (nonatomic,strong)NSMutableDictionary *%@;\n",tempStr];
}
...
+ (void)myNSDictionaryTransformToPropert:(NSString *)tempStr ToNSMutableString:(NSMutableString *)StrM withModelName:(NSString *)modelName{
    [StrM appendFormat:@"@property (nonatomic,strong)%@%@Model *%@;\n",modelName,tempStr,tempStr];
}
...
+ (void)creatProperty:(id)obj fileName:(NSString *)fileName WithContext:(NSString *)context savePath:(NSString *)savePath withNSNULL:(BOOL)NSNULL withNSDATE:(BOOL)NSDATE withNSNUMBER:(BOOL)NSNUMBER withGiveData:(NSInteger)category withModelName:(NSString *)modelName withFatherClass:(NSString *)className needEcoding:(BOOL)ecoding{
    ...
    NSMutableArray *yyArray = [NSMutableArray new];
...
//                    [CreatCoreData addStringCoreDataToNSMutableString:coreStr withName:[self specialNSString:objtemp]];
                ...
                }else if (category == 4) { //YYModel
                    
                    if (((NSArray *)obj[objtemp]).count > 0 && [((NSArray *)obj[objtemp])[0] isKindOfClass:[NSDictionary class]]) {
                        [yyArray addObject:objtemp];
                    }
    
                    [self myNSArray:[self conversionName:objtemp] ToNSMutableString:str];
                    [CreatCoreData addTransformableCoreDataToNSMutableString:coreStr withName:objtemp];
...
//                    [NSArrayOrDictionaryArr addObject:objtemp];
                    ...
                    [self myNSDictionaryTransformToPropert:myString ToNSMutableString:str withModelName:modelName];
...
//                    [self myNSMutableDictionaryJSONModel:objtemp ToNSMutableString:str];
                ...
                if(NSNULL==NO){
                    ...
                    [self myNSNull:myString ToNSMutableString:str];
                }else{
                    [self myNSString:[self specialNSString:myString] ToNSMutableString:str];
                    [CreatCoreData addStringCoreDataToNSMutableString:coreStr withName:[self specialNSString:objtemp]];
                    [stringArr addObject:[self specialNSString:objtemp]];
                }
                ...
                if(NSNUMBER==NO){
                    ...
                    switch (judge(strP)) {
                        case 1:value_int=[num intValue];
                            [self myNSNumberInt:myString ToNSMutableString:str];
                        ...
                        case 2:value_float=[num floatValue];
                            [self myNSNumberFloat:myString ToNSMutableString:str];
                ...
                [self myNSData:myString ToNSMutableString:str];
                ...
                if(NSDATE==NO){
                    [self myNSDate:myString ToNSMutableString:str];
...
#pragma mark - YYModel
                [self modelCustomToMutableString:staticText_m intArr:intArr floatArr:floatArr stringArr:stringArr ArrOrDic:NSArrayOrDictionaryArr yyArr:yyArray];
                if (yyArray.count) {
                    [self modelContainerToMutableString:staticText_m withArr:yyArray modelName:modelName];
                }
            }
...
//YYModel辅助函数
+ (void)modelCustomToMutableString:(NSMutableString *)StrM intArr:(NSArray *)intArr floatArr:(NSArray *)floatArr stringArr:(NSArray *)stringArr ArrOrDic:(NSArray *)NSArrayOrDictionaryArr yyArr:(NSArray *)yyArr
{
    [StrM appendString:@"+ (NSDictionary *)modelCustomPropertyMapper {\n"];
    [StrM appendString:@"return @{"];
    
    if (intArr.count) {
        StrM = [self joinTogetherToMutableString:StrM nameArray:intArr];
    }
    if (floatArr.count) {
        StrM = [self joinTogetherToMutableString:StrM nameArray:floatArr];
    }
    if (stringArr.count) {
...
+ (NSMutableString *)joinTogetherToMutableString:(NSMutableString *)StrM nameArray:(NSArray *)nameArray
{
    for (NSString *name in nameArray) {
        NSString *tempStr = [self conversionName:name];
        if (![tempStr isEqualToString:name]){
            [StrM appendFormat:@"@\"%@\" : @\"%@\",\n", tempStr, name];
        }
    }
    return StrM;
...
+ (void)modelContainerToMutableString:(NSMutableString *)StrM withArr:(NSArray *)arr modelName:(NSString *)modelName
{
    if (arr.count) {
...
+ (void)initNSMutableArrayWithName:(NSString *)name ToNSMutableString:(NSMutableString *)StrM{
    [StrM appendString:@"- (NSMutableArray *)"];
...
+ (void)initNSMutableDictionaryWithName:(NSString *)name ToNSMutableString:(NSMutableString *)StrM{
    [StrM appendString:@"- (NSMutableDictionary *)"];
    [StrM appendString:name];
    [StrM appendString:@"{\n\tif(!_"];
    [StrM appendString:name];
    [StrM appendString:@"){\n\t\t_"];
...
//                [StrM appendString:@"My"];
            ...
            if([tempStr hasPrefix:@"NSArray"]){
                [arrPropert addObject:[@"_" stringByAppendingString:[tempStr substringFromIndex:7]]];
            }else{
                [arrPropert addObject:[@"_" stringByAppendingString:[tempStr substringFromIndex:12]]];
            }
    ...
    if(arrOrDic.count>0){
        for (NSString *tempStr in arrOrDic) {
            if([tempStr hasPrefix:@"NSDictionary"]){
                ...
                [StrM appendFormat:@"\t[aCoder encodeObject:self.%@ forKey:@\"%@\"];\n",[tempStr substringFromIndex:7],[tempStr substringFromIndex:7]];
            }
        }
    }
    [StrM appendString:@"}\n\n"];
